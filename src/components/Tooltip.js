...
const Tooltip = (props) => {
  ...
  const tooltipClasses = classNames(
    clicked ? "flex" : "hidden",
    selected === "hover" && "group-hover:flex",
    "bg-white dark:bg-gray-130 text-gray-80 dark:text-gray-10 text-sm",
    "absolute w-28 h-28 flex-col justify-evenly items-center rounded-lg shadow-[0_10px_15px_-3px_rgba(0,0,0,0.1)]",
  ...
  return (
    <div className={classes}>
      ...
      <div className={tooltipClasses}>
        {infoHeader && <h3 className="font-semibold">{infoHeader}</h3>}
        {infoText && <p className="font-redhat">{infoText}</p>}
        {link && <a href={link} className="text-blue-60 dark:text-blue-50">Link</a>}
      </div>
    </div>
  )
}
